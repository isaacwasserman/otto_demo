...
def sign_in(email: str, password: str) -> None:
    try:
        ...
        # Save user info to session state and rerun
            ...
            st.session_state.auth_success = "You have successfully signed in"
            st.rerun()
...
def user_logged_in():
    try:
        ...
        if retrieved_user_info:
            st.session_state.user_info = retrieved_user_info
            return (
                "user_info" in st.session_state
                and len(st.session_state.user_info["users"]) == 1
                and st.session_state.user_info["users"][0]["emailVerified"]
            )
